...
export default function VistaTaskbar({ windows, activeWindowId, aeroColor, onToggleStart, onFocusWindow }: VistaTaskbarProps) {
  ...
  return (
    <nav className={`fixed inset-x-0 bottom-0 z-[120000] flex h-[46px] items-center justify-between border-t border-white/25 px-3 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] backdrop-blur-xl ${aeroColor === 'ruby' ? 'bg-[linear-gradient(180deg,rgba(114,25,42,0.9),rgba(34,7,14,0.92))]' : aeroColor === 'emerald' ? 'bg-[linear-gradient(180deg,rgba(18,93,84,0.9),rgba(5,28,25,0.94))]' : aeroColor === 'graphite' ? 'bg-[linear-gradient(180deg,rgba(62,72,85,0.9),rgba(16,20,26,0.96))]' : 'bg-[linear-gradient(180deg,rgba(40,95,160,0.9),rgba(9,25,47,0.96))]'}`}>
      <div className="flex h-full items-center gap-3">
        <button type="button" onClick={onToggleStart} className="vista-orb relative -mt-4 flex h-[58px] w-[58px] items-center justify-center rounded-full border border-white/70 text-white shadow-[0_12px_22px_rgba(0,0,0,0.4)] transition-transform hover:scale-105 active:scale-95">
